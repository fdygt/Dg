...
class CacheMiddleware:
    ...
    async def __call__(self, request: Request, call_next: callable):
        ...
        if cache_key in _cache_store:
            ...
            cached_response = _cache_store[cache_key]
            return Response(
...
def get_cache_key(request: Request) -> str:
    ...
    """
    raw = f"{request.method}-{str(request.url)}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()

async def set_cache(key: str, response: Response):
    """
...
def clear_cache(key: Optional[str] = None):
    """
    Clear specific cache by key, or all cache if key is None.
    """
    if key:

...
CURRENT_TIME = "2025-06-02 14:36:57"  # Update dengan waktu saat ini
...
async def setup_2fa(
    ...
    try:
        auth_service = AuthService(bot)
        setup_data = await auth_service.setup_2fa("fdygt")
...
async def request_password_reset(
    ...
    try:
        ...
        await auth_service.request_password_reset(
            reset_request.email,
            request_time=datetime.strptime(CURRENT_TIME, "%Y-%m-%d %H:%M:%S")
        )
        ...
        logger.error(
            f"""Password reset request error:
